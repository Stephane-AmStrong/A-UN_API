...
namespace Repository
{
    public class PaymentTypeRepository : RepositoryBase<PaymentType>, IPaymentTypeRepository
    {
        ...
        private IDataShaper<PaymentType> _dataShaper;

        public PaymentTypeRepository(
            RepositoryContext repositoryContext, 
            ISortHelper<PaymentType> sortHelper,
            IDataShaper<PaymentType> dataShaper
        ...
        public async Task<PagedList<Entity>> GetAllPaymentTypesAsync(PaymentTypeParameters paymentTypeParameters)
        {
            ...
            var shapedPaymentTypes = _dataShaper.ShapeData(sortedPaymentTypes, paymentTypeParameters.Fields);
        ...
        public async Task<Entity> GetPaymentTypeByIdAsync(Guid id, string fields)
        {
            var paymentType = FindByCondition(paymentType => paymentType.Id.Equals(id))
                .DefaultIfEmpty(new PaymentType())
                .FirstOrDefault();

            return await Task.Run(() =>
                _dataShaper.ShapeData(paymentType, fields)
            );
        }
